...
public class Elevator extends PIDSubsystem {
	...
	private SpeedController motor = new VictorSP(RobotMap.LIFTER_MOTOR_1);
	...
	public Elevator() {
		super("elevator", p, i, d, feedForward, pidPeriod);
		motor.setInverted(RobotMap.LIFTER_INVERTED);
	...
	protected void usePIDOutput(double output) {
		if (minLength < getPosition() && getPosition() < maxLength) {
			motor.set(output);
			SmartDashboard.putBoolean("Elevator at end?", false);
		} else if (getPosition() <= minLength) {
			motor.set(0.2);
			SmartDashboard.putBoolean("Elevator at end?", true);
	...
	public void setSetpoint(double angle) {
		if (minLength < angle && angle < maxLength) {
			...
			System.out.println("Setting Elevator Setpoint to " + angle);
	...
	public void setMotor(double power) {
		if (getPIDController().isEnabled()) {
			System.out.println("Elevator PID enabled; not setting motor.");
		} else {
//			if (minLength < getPosition() && getPosition() < maxLength) {
//				motor.set(power);
